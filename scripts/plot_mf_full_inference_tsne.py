#!/usr/bin/env python3
"""Infer MF-full encoder embeddings and create a t-SNE perplexity sweep."""
...
from scripts.evaluate_bp_inference_two_models import artifact_paths, checkpoint_metadata, load_inference_data  # noqa: E402
from tacs.lora import ESM2LoRAClassifier  # noqa: E402


RUN_DIR = ROOT / "outputs/tacs/mf_full_ceiling20_molecular_function_full_tacs_seed42"
INPUT = ROOT / "datasets/Swissprot/molecular_function_inference_processed.csv"
OUTPUT_DIR = ROOT / "outputs/tacs/inference_evaluation/full_ceiling20_sequence/mf_full/encoder_tsne"
LABELS = {
    0: "Other molecular-function annotations",
    1: "Toxin",
    2: "Antibiotic / Antimicrobial",
}
COLORS = ["#A0A7B4", "#E45756", "#4C78A8"]
...
def save_encoder_embeddings(args: argparse.Namespace, output_dir: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    model_config, lora_config = dict(checkpoint["model_config"]), dict(checkpoint["lora_config"])
    ...
    _, embedding_tensor = model.predict_sequences(frame["sequence"].tolist(), args.batch_size, progress_description="MF full encoder inference")
    embeddings, labels = embedding_tensor.numpy(), frame["label"].to_numpy(dtype=np.int64)
    np.savez_compressed(output_dir / "mf_full_round3_encoder_embeddings.npz", embeddings=embeddings, labels=labels)
    pd.DataFrame({"label": labels}).to_csv(output_dir / "mf_full_round3_embedding_labels.csv", index=False)
...
def plot_projection(coordinates: np.ndarray, labels: np.ndarray, perplexity: int, output_dir: Path) -> None:
    ...
    for label, name in LABELS.items():
        ...
        axis.scatter(coordinates[selected, 0], coordinates[selected, 1], s=8 if label == 0 else 26, alpha=0.28 if label == 0 else 0.88, color=COLORS[label], edgecolors="none", label=f"{name} (n={selected.sum():,})")
    axis.set(title=f"MF full inference: encoder-embedding t-SNE (perplexity {perplexity})", xlabel="t-SNE 1", ylabel="t-SNE 2")
    axis.legend(frameon=True, markerscale=1.4, loc="best", fontsize=8)
    figure.tight_layout()
    figure.savefig(output_dir / f"mf_full_encoder_tsne_perplexity_{perplexity:03d}.png", bbox_inches="tight")
    plt.close(figure)
...
def main() -> int:
    ...
    for perplexity in PERPLEXITIES:
        projection = TSNE(n_components=2, perplexity=perplexity, learning_rate="auto", init="pca", random_state=42, method="barnes_hut").fit_transform(features)
    ...
    (args.output_dir / "best_perplexity.json").write_text(json.dumps({"selection_metric": f"trustworthiness@{args.trustworthiness_neighbors}", "best": best}, indent=2) + "\n", encoding="utf-8")
