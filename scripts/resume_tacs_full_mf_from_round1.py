#!/usr/bin/env python3
"""Resume the interrupted MF full-ceiling20 run after completed round 1."""
...
DEFAULT_CONFIG = ROOT / "configs/tacs_molecular_function_full_lora.yaml"
DEFAULT_RUN = ROOT / "outputs/tacs/mf_full_ceiling20_molecular_function_full_tacs_seed42"
COMPLETED_ROUND = 1
...
def validate(config: dict, run_dir: Path) -> dict:
    if config["data"]["task"] != "molecular_function":
        raise ValueError("This recovery command is only for the interrupted MF run.")
    if config["experiment"]["name"] != "mf_full_ceiling20":
